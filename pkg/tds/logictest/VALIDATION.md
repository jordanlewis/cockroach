...
Generated: 2026-04-05 (validation pass #7)
...
|--------|-------|
| Total test files | 43 |
| Total test directives (exec/query) | 906 |
| Passing directives | 876 (97%) |
| Error directives (expected failures) | 30 (3%) |
| Known behavioral divergences | 2 |

All 43 test files PASS. Zero unexpected failures.

**Changes from pass #6 (2026-04-05):**
- EXEC with named parameters FIXED: catalog regex no longer intercepts
  `@param=value` syntax, allowing named params to flow through the
  parser/executor path where they are handled correctly
- Translator no longer errors on ExecStmt (dead code path — EXEC is
  dispatched directly from execParsedStmt to execStoredProcedure)
- 2 new test files: cursors (5 directives), stored_procedures (16 directives)
- exec_patterns extended with 4 named parameter tests for known system procs
- VALIDATION.md corrected: exec_patterns:93,97 are unknown-procedure errors,
  not named-parameter errors (previous categorization was incorrect)
...
## Error Categorization (30 total)

### Expected Errors (correct behavior) — 13
...
| exec_patterns | 93 | unsupported | EXEC unknown procedure |
| exec_patterns | 97 | unsupported | EXEC unknown procedure |

Note: The 2 "invalid integer" errors are a parser limitation (unary minus
on int64 max+1 overflows). Low priority.

### Intentional Feature Errors (parsed but unsupported) — 5
...
### Unsupported Feature Gap Tests — 4

These tests document features that are not yet supported, producing
expected errors. They serve as coverage for future implementation.

| File | Line | Error | Feature |
|------|------|-------|---------|
| tsql_control_flow | 129 | unsupported | EXEC unknown proc with named params |
| ground_truth_system | 162 | unsupported | EXEC non-existent procedure |
| stored_procedures | 12 | unsupported | CREATE PROCEDURE |
| stored_procedures | 16 | unsupported | CREATE PROCEDURE with schema prefix |

### Cursor Gap Tests (all parse errors) — 5

Cursors are not implemented. These tests document the parser limitations.

| File | Line | Error | Feature |
|------|------|-------|---------|
| cursors | 21 | parse error | DECLARE CURSOR |
| cursors | 27 | parse error | OPEN cursor |
| cursors | 33 | parse error | FETCH NEXT |
| cursors | 39 | parse error | CLOSE cursor |
| cursors | 45 | parse error | DEALLOCATE cursor |

### Stored Procedure Gap Tests — 3

| File | Line | Error | Feature |
|------|------|-------|---------|
| stored_procedures | 68 | unsupported | EXEC unknown proc |
| stored_procedures | 72 | unsupported | EXEC unknown proc with named params |
| stored_procedures | 79 | parse error | EXEC with OUTPUT parameter |
...
### EXEC Named Parameters (FIXED in pass #7)

EXEC with named parameters (`EXEC proc @param=value`) now works for all
known system procedures (sp_tables, sp_columns, sp_helptext). The catalog
regex layer was intercepting named parameter syntax and misinterpreting
`@param='value'` as a literal argument. Fixed by excluding `@`-prefixed
arguments from catalog regex matching, allowing named params to flow
through the parser/executor path which already handles them correctly.
...
| cte_derived | 8 | 6 | 14 | 0 | CTEs and derived tables |
| cursors | 5 | 0 | 5 | 5 | Cursor gap coverage |
| datetime_extended | 7 | 9 | 16 | 0 | Date/time operations |
...
| exec_patterns | 16 | 0 | 16 | 2 | EXEC stored procedure patterns |
...
| identity_rowcount | 15 | 5 | 20 | 0 | @@IDENTITY and @@ROWCOUNT |
| joins | 12 | 10 | 22 | 0 | JOIN operations |
| merge_extended | 14 | 6 | 20 | 0 | Extended MERGE patterns |
| multi_row | 6 | 10 | 16 | 0 | Multi-row operations |
| null_handling | 0 | 5 | 5 | 0 | NULL semantics |
| output_clause | 8 | 3 | 11 | 0 | OUTPUT clause |
...
| stored_procedures | 12 | 0 | 12 | 4 | Stored procedure patterns |
| string_ops | 6 | 15 | 21 | 0 | String operations |
| subqueries | 5 | 9 | 14 | 0 | Subquery patterns |
| sybase_compat | 9 | 7 | 16 | 0 | Sybase ASE compatibility |
...
| variables_extended | 10 | 5 | 15 | 0 | Extended variables |
| window_functions | 7 | 11 | 18 | 0 | Window functions |
...
1. **CREATE PROCEDURE/FUNCTION/TRIGGER** (3 errors) — ddl_extended.
   These are parsed but intentionally unsupported.

2. **PIVOT/UNPIVOT** (2 errors) — select_extended:250,256.
   Parsed but translation not implemented.

3. **Cursor support** (5 errors) — cursors. Not parsed at all.
   Requires parser extensions for DECLARE CURSOR, OPEN, FETCH, CLOSE,
   DEALLOCATE.
...
All 43 test files PASS. The test suite is at fixed point — no regressions
remain and all new tests (pass #7) pass on first run. Remaining error
