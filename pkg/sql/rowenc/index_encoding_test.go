...
func makeTableDescForTest(test indexKeyTest) (*tabledesc.Immutable, util.FastIntMap) {
	...
	for i, test := range tests {
		...
		checkEntry := func(index *descpb.IndexDescriptor, entry kv.KeyValue) {
			...
			for j, value := range values {
				idx, err := colMap.GetOrError(int(index.ColumnIDs[j]))
				if err != nil {
					t.Fatal(err)
				}

				testValue := testValues[idx]
...
func ExtractIndexKey(
	...
	for i, columnID := range index.ColumnIDs {
		colMap.Set(int(columnID), i)
	}
	for i, columnID := range index.ExtraColumnIDs {
		colMap.Set(int(columnID), i+len(index.ColumnIDs))
	}
