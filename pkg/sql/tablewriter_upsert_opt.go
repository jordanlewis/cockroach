...
type optTableUpserter struct {
	...
	// rowsNeeded is true.
	colIDToReturnIndex util.FastIntMap
	...
	if tu.rowsNeeded {
		...
		if tu.ri.InsertColIDtoRowIndex.Len() == tu.colIDToReturnIndex.Len() {
			for i := range tu.ri.InsertCols {
				...
				insertIndex, err := tu.ri.InsertColIDtoRowIndex.GetOrError(int(colID))
				if err != nil {
					return err
				}
				resultIndex, ok := tu.colIDToReturnIndex.Get(int(colID))
...
func (tu *optTableUpserter) makeResultFromRow(
	row tree.Datums, colIDToRowIndex util.FastIntMap,
) tree.Datums {
	resultRow := make(tree.Datums, tu.colIDToReturnIndex.Len())
	tu.colIDToReturnIndex.ForEach(func(colID, returnIndex int) {
